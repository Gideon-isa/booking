...
// NewTesRepo creates a new repository
func NewTestRepo(a *config.AppConfig) *Respository {
	return &Respository{
		App: a,
...
func (m *Respository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	err := r.ParseForm()
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "can't get parse form")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "can't inser room restriction from session")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
...
// ShowLogin handles the login page
...
// AdminAllReservations shows all the reservations in admin tool
func (m *Respository) AdminAllReservations(w http.ResponseWriter, r *http.Request) {
...
// AdminNewReservations shows all new reservations in admin too
func (m *Respository) AdminNewReservations(w http.ResponseWriter, r *http.Request) {
...
func (m *Respository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
...
func (m *Respository) AdminPostShowReservation(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		helpers.ServerError(w, err)
		return
...
func (m *Respository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
...
func (m *Respository) AdminProcessReservation(w http.ResponseWriter, r *http.Request) {
	id, _ := strconv.Atoi(chi.URLParam(r, "id"))
	src := chi.URLParam(r, "src")
	_ = m.DB.UpdateProcessedForReservation(id, 1)
	m.App.Session.Put(r.Context(), "flash", "Reservation markd as processed")

	http.Redirect(w, r, fmt.Sprintf("/admin/reservations-%s", src), http.StatusSeeOther)
}

// AdminDeleteReservation deletes a reservation
func (m *Respository) AdminDeleteReservation(w http.ResponseWriter, r *http.Request) {
	id, _ := strconv.Atoi(chi.URLParam(r, "id"))
	src := chi.URLParam(r, "src")
	_ = m.DB.DeletedReservation(id)
...
func (m *Respository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		helpers.ServerError(w, err)
		return
	...
	for _, x := range rooms {
		...
		for name, value := range curMap {
			// ok will be false if the the value is not in the map
